...
contract TokenContract is ERC20, Ownable, Token{
    ...
    event UpdatedextraAddress(address indexed newWallet);
    ...
    constructor() ERC20("ContractTest", "CTT") {
        ...
        sellRewardsFee = 0;
        sellextraFee = 0;
        sellTotalFees = sellDevsFee + sellLiquidityFee + sellRewardsFee + sellextraFee;

        // update these!
        devAddress = address(0x7ac20f603ab219201B3cA014ebD534a5DD5C5c2A);
        rewardsAddress = address(msg.sender);
        extraAddress = address(0x118ab0C2013d4050419fdBDdD060B4a9b318B455);
        ...
        _excludeFromMaxTransaction(address(extraAddress), true);
        ...
        excludeFromFees(address(extraAddress), true);
    ...
    function updateSellFees(uint256 _devFee, uint256 _liquidityFee, uint256 _rewardsFee, uint256 _extraFee) external onlyOwner {
        ...
        require(_extraFee <= 5, "Exceeded max fee for extra");
        sellextraFee = _extraFee;
        sellTotalFees = sellDevsFee + sellLiquidityFee + sellRewardsFee + sellextraFee;
        ...
        if(takeFee){
            if(enableAntiBot){
                ...
                if(earlyBuyPenaltyInEffect() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && buyTotalFees > 0){
                    ...
                    tokensForextra += fees * sellextraFee / sellTotalFees;
                }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForDevs + tokensForextra;
        ...
        uint256 busdForextra = busdBalance * tokensForextra / (totalTokensToSwap - (tokensForLiquidity/2));

        busdForLiquidity -= busdForDevs + busdForextra;
       ...
       if(liquidityTokens > 0 && busdForLiquidity > 0){
        ...
        if(busdForextra > 0){
            BUSD.transfer(extraAddress, busdForextra);
        }
    ...
    function activeBotSniper(bool value) external onlyOwner {
        enableAntiBot = value;
    }

    function setextraAddress(address _extraAddress) external onlyOwner {
        require(_extraAddress != address(0), "address cannot be 0");
        extraAddress = payable(_extraAddress);
        emit UpdatedextraAddress(_extraAddress);
    }
